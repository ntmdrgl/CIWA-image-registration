...
class FlirImageExtractor:
    ...
    def get_atmospheric_temperature(self):
        """
        Return the atmospheric temperature from the metadata
        :return: float
        """
        if self.metadata is None:
            raise ValueError("Metadata not extracted yet. Call process_image() first.")
        return FlirImageExtractor.extract_float(self.metadata['AtmosphericTemperature'])

    def extract_metadata(self):
        """
        Extract metadata from the FLIR image without processing the thermal image
        :return: dict
        """
        meta_json = subprocess.check_output(
    ...
    def extract_thermal_image(self):
        ...
        meta_json = subprocess.check_output(
            [self.exiftool_path, self.flir_img_filename, '-Emissivity', '-SubjectDistance', '-AtmosphericTemperature',
             ...
             '-PlanckR1', '-PlanckB', '-PlanckF', '-PlanckO', '-PlanckR2', '-j'])
        meta = json.loads(meta_json.decode())[0]
        self.metadata = meta  # Store the metadata
